...
# if C.dataset == 'AVA':
shared_layers_orig = Input(shape=(None,None,None,832), name = 'shared_layers_orig')
classifier = nn.classifier_i3d_concat(shared_layers_orig, 1, nb_classes=num_classes, trainable=True, version=version)

model_classifier = Model([shared_layers_orig], classifier)

# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
model_all = Model([shared_layers_orig], classifier)
...
for epoch_num in range(num_epochs):
	...
	while True:
		try:
			...
			loss_class = model_classifier.train_on_batch([i3d_f], [Y1[:, :, :]])
