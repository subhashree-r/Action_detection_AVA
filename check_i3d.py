...
def conv3d_bn(x,
    ...
    x = TimeDistributed(Conv3D(
        filters, (num_frames, num_row, num_col),
        strides=strides,
        padding=padding,
        use_bias=use_bias,
        name=conv_name),input_shape = (4,20,14,14,832))(x)
...
def Inception_Inflated3d(include_top=True,
    ...
    #
    # # Mixed 4f
    # branch_0 = conv3d_bn(x2, 256, 1, 1, 1, padding='same', name='Conv3d_4f_0a_1x1')
    #
    # branch_1 = conv3d_bn(x2, 160, 1, 1, 1, padding='same', name='Conv3d_4f_1a_1x1')
    # branch_1 = conv3d_bn(branch_1, 320, 3, 3, 3, padding='same', name='Conv3d_4f_1b_3x3')
    #
    # branch_2 = conv3d_bn(x2, 32, 1, 1, 1, padding='same', name='Conv3d_4f_2a_1x1')
    # branch_2 = conv3d_bn(branch_2, 128, 3, 3, 3, padding='same', name='Conv3d_4f_2b_3x3')
    #
    # branch_3 = MaxPooling3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_4f_3a_3x3')(x2)
    # branch_3 = conv3d_bn(branch_3, 128, 1, 1, 1, padding='same', name='Conv3d_4f_3b_1x1')
    #
    # x1 = layers.concatenate(
    #     [branch_0, branch_1, branch_2, branch_3],
    #     axis=channel_axis,
    #     name='Mixed_4f')
    #
    # print ("Shape is", K.shape(x1))
    # Downsampling (spatial and temporal)

    inp = Input(shape=(4,16,14,14,832))
    # x = MaxPooling3D((2, 2, 2), strides=(2, 2, 2), padding='same', name='MaxPool2d_5a_2x2')(x1)

    # Mixed 5b
    branch_0 = conv3d_bn(inp, 256, 1, 1, 1, padding='same', name='Conv3d_5b_0a_1x1')

    branch_1 = conv3d_bn(inp, 160, 1, 1, 1, padding='same', name='Conv3d_5b_1a_1x1')
    branch_1 = conv3d_bn(branch_1, 320, 3, 3, 3, padding='same', name='Conv3d_5b_1b_3x3')

    branch_2 = conv3d_bn(inp, 32, 1, 1, 1, padding='same', name='Conv3d_5b_2a_1x1')
    branch_2 = conv3d_bn(branch_2, 128, 3, 3, 3, padding='same', name='Conv3d_5b_2b_3x3')

    branch_3 = TimeDistributed(MaxPooling3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_5b_3a_3x3'))(inp)
    ...
    branch_3 = TimeDistributed(MaxPooling3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_5c_3a_3x3'))(x)
    ...
    if include_top:
        ...
        print("Mixed 5c", x1.shape.as_list())
        x = TimeDistributed(AveragePooling3D((2, 14, 14), strides=(1, 1, 1), padding='valid', name='global_avg_pool'))(x1)
        x = TimeDistributed(Dropout(dropout_prob))(x)
        #
        x = conv3d_bn(x, classes, 1, 1, 1, padding='same',
                use_bias=True, use_activation_fn=False, use_bn=False, name='Conv3d_6a_1x1')
        #
        num_frames_remaining = int(x.shape[2])
        x = TimeDistributed(Reshape((num_frames_remaining, classes)))(x)

        # logits (raw scores for each class)
        x = TimeDistributed(Lambda(lambda x: K.mean(x, axis=1, keepdims=False),
                   output_shape=lambda s: (s[0], s[2])))(x)

        # if not endpoint_logit:

        x = TimeDistributed(Activation('softmax', name='prediction'))(x)
        # out_regr = TimeDistributed(Activation('linear', name='prediction'))(x)
        print("Final shape of final layer",x.shape.as_list())
    else:
        h = int(x.shape[2])
        w = int(x.shape[3])
    ...
    # inputs = x1
    # create model
    model = Model(inp, x, name='i3d_inception')
    ...
    #     downloaded_weights_path = '/home/subha/hoi_vid/keras-kinetics-i3d/models/rgb_inception_i3d_kinetics_only_tf_dim_ordering_tf_kernels_no_top.h5'
    ...
    #
    # model_new = Model(img_input, x2)
    # print("The final layer shae is",x2.shape.as_list())
    return model
