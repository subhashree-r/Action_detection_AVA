...
def Inception_Inflated3d(include_top=True,
    ...
    if include_top:
        ...
        x4 = conv3d_bn(x, classes, 1, 1, 1, padding='same',
                use_bias=True, use_activation_fn=False, use_bn=False, name='Conv3d_6a_1x1')
        # print("Reshape sizes 1 are",x.shape.as_list())
        num_frames_remaining = int(x.shape[1])
        # print(num_frames_remaining)
        x3 = Reshape((num_frames_remaining, classes))(x4)
        # print("Reshape sizes 2 are",x.shape.as_list())
    ...
    weights_path = '/home/subha/hoi_vid/keras-kinetics-i3d/models/rgb_inception_i3d_kinetics_only_tf_dim_ordering_tf_kernels_no_top.h5'
    model.load_weights(weights_path)
    model_new = Model(inputs, x1, name='i3d_inception_features')
    # weights_list = model_new.get_weiss()
    # for i in range(42):
    #     model.pop()
    # for i in range(len(weights_list)):
    #     # if all(weights_list[i]==weights_list_1[i]):
    #     #     print("yes")
    #     # else:
    #     #     print("no")
    #     if np.array_equal(weights_list[i], weights_list_1[i]):
    #         print("yes")
    # weights_list = model.get_weights()
    # print(len(model_new.layers), len(model.layers))
    # for i, weights in enumerate(weights_list[1:len(model_new.layers)]):
    #     model_new.layers[i].set_weights(weights)
        # print(len(weights),model_new.layers[i])
    ...
    '''
    # model_new = Model(img_input, x3)
    # print("The final layer shae is",x3.shape.as_list())
    return model_new
