...
set_session(sess)  # set this TensorFlow session as the default session for Keras1000
...
classifier = nn.classifier_i3d(shared_layers_orig, roi_input, 1, nb_classes=len(classes_count), trainable=True)
model_classifier = Model([roi_input, vid_input], classifier)

# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
model_all = Model([vid_input, roi_input], classifier)
plot_model(model_all, to_file='model_all_i3d_cls_latest.png', show_shapes = True)
