...
parser.add_option("-i", "--vis_folder", type="str", dest="vis_folder",
				help="Number of ROIs per iteration. Higher means more memory use.", default='ava_val_subset_80.csv')
...
if version == 'concat' and type == 'v1':
	classifier = 	classifier = nn.classifier_i3d_concat_new(feature_map_input, shared_layers_image, roi_input, 1, nb_classes=8, trainable=True)
	...
	model_classifier = Model([img_input, feature_map_input, roi_input], classifier)
# elif version == 'whole' and type == 'v2':
# 	classifier = nn.classifier_i3d_concat(feature_map_input, 1, nb_classes=len(classes_count), trainable=True, version=version)
# 	model_classifier = Model([feature_map_input], classifier)
...
for i in tqdm(range(len(df))):
	...
	try:
				...
				fp = filepath.replace('keyframes','numpy_8_actions')
				...
				shared_layers_orig = np.load(fp)
				# seq = get_frame_idx(filepath)
				#
				# # print filepath, seq
				# vid_numpy = []
				x_img_orig = cv2.resize(img, (resized_width, resized_height), interpolation=cv2.INTER_CUBIC)
				x_img = np.expand_dims(x_img_orig, axis = 0)
				# # print filepath, seq
				# # bp()
				# for frame in seq:
				# 	if frame!=-1:
				...
				real_x1, real_y1, real_x2, real_y2 = row[2]*320, row[3]*400, row[4]*320, row[5]*400
				cv2.rectangle(x_img_orig,(int(real_x1),int(real_y1)),(int(real_x2),int(real_y2)), (255,255,255), 2)
				# ind =np.argmax(P_cls[0,:])
				gt = row[-2]
				pred_prob = P_cls[0,:]
				# pred_class = class_mapping[ind]
				sort_prob = sorted(range(len(pred_prob)), key=lambda i: pred_prob[i])[-4:]
				ac_pred = [class_mapping[ind] for ind in sort_prob]
				ac_pred = map(int, ac_pred)
				# print ac_pred
				# bp()
				# print gt, pred_class
				if int(gt) in ac_pred:
					# pred_prob = P_cls[0,:]
					# sort_prob = sorted(range(len(pred_prob)), key=lambda i: pred_prob[i])[-3:]
					# prob_str = [str(pred_prob[ind]) for ind in sort_prob]
					# prob_str = ','.join(prob_str)
					#
					# ac_pred = [class_mapping[ind] for ind in sort_prob]
					...
					# lineType               = 2
					#
					# cv2.putText(img,','.join(ac_pred)+'-'+prob_str,
					# 	bottomLeftCornerOfText,ff
					# 	font,
					# 	fontScale,
					# 	fontColor,
					# 	lineType)
					seq_name = filepath.split(os.path.sep)[-2]
					vid_dir = os.path.join(vis_folder,str(row[-2]),'true_positives', seq_name)
					if not os.path.isdir(vid_dir):
						os.makedirs(vid_dir)


					save_img_name = os.path.join(vid_dir,img_name)
					# print save
					cv2.imwrite(save_img_name, x_img_orig)
				else:
					seq_name = filepath.split(os.path.sep)[-2]
					vid_dir = os.path.join(vis_folder,str(row[-2]),'false_positives', seq_name)
					if not os.path.isdir(vid_dir):
						os.makedirs(vid_dir)
