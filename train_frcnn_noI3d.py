...
parser.add_option("-v", "--version",dest="version", help="Output path for weights.", default='v1')
...
data_gen_train = data_generators.get_i3d_feature(train_imgs, classes_count, C, nn.get_img_output_length, K.image_dim_ordering(), mode='train')
data_gen_val = data_generators.get_i3d_feature(val_imgs, classes_count, C, nn.get_img_output_length,K.image_dim_ordering(), mode='val')
...
# if C.dataset == 'AVA':
i3d_features = Input(shape=(None,None,None,832), name = 'i3d_features')
if version == 'v1':
	classifier = nn.classifier_i3d_concat_new(i3d_features, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True)
	model_classifier = Model([img_input, roi_input, i3d_features], classifier)
	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
	model_all = Model([img_input, roi_input,i3d_features], classifier)

elif version == 'roi':
	classifier = nn.classifier_i3d(i3d_features,  roi_input, 1, nb_classes=num_classes)
	model_classifier = Model([roi_input, i3d_features], classifier)

	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
	model_all = Model([i3d_features, roi_input], classifier)
# elif version == 'frcnn':
# 	classifier = nn.classifier(i3d_features,  roi_input, 1, nb_classes=num_classes)
# 	model_classifier = Model([roi_input, i3d_features], classifier)
#
# 	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
# 	model_all = Model([i3d_features, roi_input], classifier)
	...
	classifier = nn.classifier_i3d_concat_v2(i3d_features, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True)
	model_classifier = Model([img_input, roi_input, i3d_features], classifier)
	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
	model_all = Model([img_input, roi_input,i3d_features], classifier)
#
# classifier = nn.classifier_i3d_concat_new(shared_layers_orig, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True, version = version)
#
# classifier = nn.classifier_i3d_concat_new(i3d_features, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True)

plot_model(model_all, to_file='model_all_i3d_cls.png', show_shapes = True)
...
for epoch_num in range(num_epochs):
	...
	while True:
		try:
			img_data, i3d_f, x_img = next(data_gen_train)
			X2, Y1 = roi_helpers.calc_iou(img_data, C, class_mapping)

			num+=1
			if version=='roi':
				loss_class = model_classifier.train_on_batch([X2[:, :, :],i3d_f], [Y1[:, :, :]])
			else:
				loss_class = model_classifier.train_on_batch([x_img,X2[:, :, :],i3d_f], [Y1[:, :, :]])
			...
			if iter_num == epoch_length:
				...
				while True:
						# try:
						img_data, i3d_f, x_img = next(data_gen_val)
						# print("validation")
						X2, Y1 = roi_helpers.calc_iou(img_data, C, class_mapping)
						if version=='roi':
							loss_class = model_classifier.train_on_batch([X2[:, :, :],i3d_f], [Y1[:, :, :]])
