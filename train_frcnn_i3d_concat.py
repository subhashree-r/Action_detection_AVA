...
with open('classes_count.pickle', 'wb') as config_f:
	pickle.dump(classes_count,config_f)
...
if version == 'v1':
	classifier = nn.classifier_i3d_concat_new(shared_layers_orig, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True)
	model_classifier = Model([img_input, roi_input, vid_input], classifier)

	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
	model_all = Model([img_input,vid_input, roi_input], classifier)

elif version == 'roi':
	classifier = nn.classifier_i3d(shared_layers_orig,  roi_input, 1, nb_classes=num_classes)
	model_classifier = Model([roi_input, vid_input], classifier)

	# this is a model that holds both the RPN and the classifier, used to load/save weights for the models
	model_all = Model([vid_input, roi_input], classifier)
else:
	print "Using version 2"
	classifier = nn.classifier_i3d_concat_v2(shared_layers_orig, shared_layers_image, roi_input, 1, nb_classes=num_classes, trainable=True)
	model_classifier = Model([img_input, roi_input, vid_input], classifier)
...
plot_model(model_all, to_file='i3d_model.png', show_shapes = True)
