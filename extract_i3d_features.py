...
#
# imgs = train_imgs+val_imgs
#
# print('Num train samples {}'.format(len(train_imgs)))
# print('Num val samples {}'.format(len(val_imgs)))


# data_gen_train = data_generators.get_anchor_gt(imgs, classes_count, C, nn.get_img_output_length, K.image_dim_ordering(), mode='train')
...
def extract_numpy_single_frame(img):
...
def get_frames_index(total_list,frame_idx,winSize):
	nb = (winSize/2)-1
	na = (winSize/2)+1
	final_frame_idx = len(total_list)-1
	sf = final_frame_idx-(winSize/2)+1
	ef = len(total_list)
	seq = []
	if frame_idx < (winSize/2)-1:
		zp_frames = nb - frame_idx
		seq = [-1 for number in range(int(zp_frames))]
		seq.extend(range(0,int(frame_idx)+1))
		seq.extend(range(int(frame_idx)+1, int(frame_idx)+int(na)))
		if len(seq)!=winSize:
			print "No"
	elif frame_idx >= (winSize/2)-1 and frame_idx < sf:
		start_index = frame_idx-(winSize/2)+1
		end_index = (winSize/2) + frame_idx
		# print frame_idx
		seq = range(int(start_index),int(end_index+1))

	elif frame_idx >=sf and frame_idx <ef:
		sa = frame_idx-(nb)
		seq.extend(range(int(sa),int(ef)))
		zp = winSize-len(seq)
		# print zp
		seq.extend([-1 for _ in range(int(zp))])
		if len(seq)!=winSize:
			print "No"
	return seq
...
csv_file = '/work/newriver/subha/AVA_dataset/ava-dataset-tool/ava_val_8_actions.csv'
f1 = open(csv_file,'r')
lines = f1.read().splitlines()
for line in tqdm(lines):
			# while True:
			# try:
			# img_data, seq_numpy, x_img = next(data_gen
			# _train)
			tags = line.split(',')
			filepath = os.path.join(train_path,tags[0],str(int(tags[1]))+'.jpg')
			# print filepath
			...
			for frame in seq:
				if frame!=-1:
						...
						img_list.append(fr_img)
				else:
					img_list.append(np.zeros((resized_height,resized_width,3)))

			# if augment:
			# 	try:
			# 		img_list, img_data_aug, new_img = ind_aug_data(img_list, img_data_aug, img)
			# 		img = new_img
			# 	except:
			# 		pass
			# if random_crop:
			...
			resized_width = 320
			resized_height = 400
			for frame,im_frame in zip(seq,img_list):
				if frame!=-1:
						# fr_name = os.path.join(img_folder, frame)
						# fr_img = cv2.imread(fr_name)
						# if random_crop:
						# 	fr_img = fr_img[rc[1]:rc[3], rc[0]:rc[2]]
						fr_img =  cv2.resize(im_frame, (resized_width, resized_height), interpolation=cv2.INTER_CUBIC)
						fr_npy = extract_numpy_single_frame(fr_img)
						# print fr_npy.shape
			...
			fp = filepath
			# # print fp
			fp = fp.replace('keyframes','numpy_8_actions')
			fp = fp.replace('.jpg','.npy')
			# print fp
			numpy_dir = '/'.join(fp.split('/')[:-1])
			if not os.path.isdir(numpy_dir):
				os.makedirs(numpy_dir)
			# # print numpy_dir
			i3d_features = rgb_model.predict(vid_numpy)
			# # print i3d_features.shape
			np.save(fp,i3d_features)
			# print
'''
X2, Y1 = roi_helpers.calc_iou(img_data, C, class_mapping)
# img_features =
loss_class = model_classifier.train_on_batch([x_img,X2[:, :, :],seq_numpy], [Y1[:, :, :]])
...
if iter_num == epoch_length:
	...
	while True:
			# try:
			img_data, seq_numpy = next(data_gen_val)
			print("validation")
			X2, Y1 = roi_helpers.calc_iou(img_data, C, class_mapping)
			loss_class = model_classifier.train_on_batch([ X2[:, :, :],seq_numpy], [Y1[:, :, :]])
