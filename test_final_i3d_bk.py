...
parser.add_option("--config_filename", dest="config_filename", help=
				...
				default="config_noI3d_cheating8_AVA.pickle")
...
output_csv_file = os.path.join('evaluation','phase2',options.output)
...
feature_map_input = Input(shape=(None, None, None,None,832))
...
# classifier = nn.classifier_i3d_concat_new(shared_layers_input, 1, nb_classes=num_classes, trainable=True)
...
for i in tqdm(range(len(df))):
	try:
		...
		print shared_layers_orig.shape
		s3 = time.time()
		[P_cls]= model_classifier.predict([shared_layers_orig, rois])
