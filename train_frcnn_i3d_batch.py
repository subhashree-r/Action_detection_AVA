...
val_indices = range(len(val_imgs))
data_gen_train = data_generators.get_anchor_gt_batch(train_imgs, classes_count, C, nn.get_img_output_length, K.image_dim_ordering(),indices, mode='train')
data_gen_val = data_generators.get_anchor_gt_batch_val(val_imgs, classes_count, C, nn.get_img_output_length,K.image_dim_ordering(), val_indices,mode='val')

roi_input = Input(shape=(None, 4),name = 'roi_input')
vid_input = Input(shape =(64, 400, 320, 3),name = 'vid_input')
...
print shared_layers_orig.shape.as_list()
# bp()
...
classifier = nn.classifier_i3d_batch(shared_layers_orig, roi_input, 1, nb_classes=len(classes_count), trainable=True)
# model_rpn = Model(vid_input, rpn[:2])
...
plot_model(model_all, to_file='model_all_i3d_cls_batch.png', show_shapes = True)
...
for epoch_num in range(num_epochs):
	...
	while True:
		try:
			...
			print(seq_numpy.shape, x_img.shape)
			# print type(img_data)
			X2, Y1 = roi_helpers.calc_iou_batch(img_data, C, class_mapping)
